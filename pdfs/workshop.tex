...
\date{Workshop PowDev -- March 2026}
...
\begin{enumerate}
  ...
  \item \textbf{Criticality index \applink{app:L}{App.~L: Criticality}}\\$\mathrm{Crit}(s) = \alpha\,\mathrm{Stress} + (1{-}\alpha)\,\mathrm{Hard}$
\end{enumerate}
\end{column}
...
\vspace{0.1em}
\applink{app:W}{App.~W: Simple}
\applink{app:X}{App.~X: Critical}
\vspace{4em}
...
{\small $\min_{u,x}\; C^{\mathrm{gen}} + C^{\mathrm{su}} + C^{\mathrm{dr}} + C^{\mathrm{uns}} + C^{\mathrm{spl}} + C^{\mathrm{sto}} + C^{\mathrm{imp}} + C^{\mathrm{ovg}}$} \applink{app:A}{App.~A: Objective}
\vspace{0.3em}
...
\begin{itemize}
  ...
  \item \applink{app:B}{App.~B: Variables}
\end{itemize}
\vspace{0.4em}
\textbf{Label quality:}
\begin{itemize}
  ...
  \item \applink{app:Y}{App.~Y: Dispatch comparison}
\end{itemize}
\vspace{0.2em}
...
\applink{app:AB}{App.~AB: Temporal}
\end{column}
\begin{column}{0.48\textwidth}
...
{\scriptsize (A) Pipeline vs MILP solve time (log--log; dashed = parity). (B) Cost gap vs criticality. (C) Speedup vs criticality.}

\applink{app:P}{App.~P: Central Divergence}
...
\begin{itemize}
  \item {\scriptsize100\% faster (median $5.4\times$, range $3.9$--$9.3\times$)}
  \item {\scriptsize 100\% feasible, near-zero slack}
  \item {\scriptsize 48\% hard-fix (EBM transfers well)}
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\vspace{0.2em}
...
\begin{itemize}
  \item {\scriptsize Cost gap 119.6\% (EBM under-represented)}
  \item {\scriptsize MILP also returns sub-optimal incumbents}
  \item {\scriptsize Feasible plans in $<$4 min even OOD}
\end{itemize}
\end{column}
\end{columns}
...
\begin{enumerate}
  ...
  \vspace{0.5em}
  \item[] {\scriptsize All computable ex ante (no MILP solve). Normalised to $[0,1]$ via min-max.}
\end{enumerate}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{enumerate}
  ...
  \item[] \backlink{main:scengen}{Block 1: Scenarios}
\end{enumerate}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{App.\ P: Solve Time, Cost Gap}
\hypertarget{app:P}{}
\centering
...
{\scriptsize Overview of the heterogeneous multi-layer graph.}
\backlink{main:hte}{Block 3: Graph \& HTE}
\end{frame}

\begin{frame}{App.\ AB: Temporal Graph}
\hypertarget{app:AB}{}
\centering
\includegraphics[width=0.92\textwidth, height=0.78\textheight, keepaspectratio]{fig02_temporal_overview.png}

{\scriptsize Temporal structure of a given zone over time.}
\backlink{main:hte}{Block 3: Graph \& HTE}
...
\begin{frame}[allowframebreaks]{References}
    \footnotesize
    \renewcommand{\refname}{} % removes inner "References"
    \nocite{*}
